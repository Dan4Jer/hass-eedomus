...
def get_api_token(username, password):
    ...
    try:
        # Create a temporary config file for the API call
        config_data = {
            "type": "create_token",
            "data": {
                "client_id": "eedomus-history-checker",
                "client_secret": "history-checker-secret"
            }
        }
        ...
        cmd = [
            'curl', '-s', '-X', 'POST',
            'http://localhost:8123/api/auth/login_flow',
        ...
        if result.returncode == 0:
            ...
            if response.get('result') == 'ok':
                return response.get('data', {}).get('refresh_token')
        
        print(f"❌ Erreur lors de l'obtention du token: {result.stderr}")
        return None
...
def check_history_sensors(api_token):
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"❌ Erreur lors de la récupération des états: {result.stderr}")
            return None
