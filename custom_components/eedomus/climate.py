        ...
        for child_periph_id, child_periph in all_peripherals.items():
            if child_periph.get("parent_periph_id") == self._periph_id:
                if child_periph.get("usage_id") == "7":  # Temperature sensor
                    ...
                    if child_value and child_value.replace(".", "").replace("-", "").isdigit():
    ...
    async def async_set_temperature(self, **kwargs):
        ...
        try:
            ...
            if usage_id == "15":
                ...
                _LOGGER.debug("Setting %s temperature directly to %.1f°C (usage_id=15)", 
                            self._attr_name, rounded_temp)
            else:
            ...
            if eedomus_value is None:
                _LOGGER.error("Could not determine eedomus value for temperature %.1f°C", temperature)
                return
